...
- **AT-10의 남은 절반(installer 자체의 clean 환경 검증)을 CI에 편입했다.** `windows-release.yml`에 Inno Setup 컴파일 → silent 설치 → 설치된 위치에서 `--self-test` → 시작 메뉴 바로가기 확인 → silent 제거까지 자동으로 수행하는 단계를 추가했다(빌드된 `setup.exe`는 `GenomeWorkbench-installer` artifact로 업로드됨). 이 러너는 이 개발 머신과 달리 아무것도 미리 설치되어 있지 않은 진짜 clean 환경이므로, "완전히 별도의 clean Windows 환경에서의 installer 검증"이라는 AT-10의 요구를 (별도 사용자 계정/영구 VM은 아니지만) 매 릴리스마다 자동으로 반복 검증하는 형태로 충족한다. **이 워크플로우 변경 자체는 이 보고서 갱신 시점에 아직 실제로 실행해 통과를 확인하지 못했다** — push 후 Actions 실행 결과를 반드시 확인할 것(성공하면 이 문단을 갱신).
