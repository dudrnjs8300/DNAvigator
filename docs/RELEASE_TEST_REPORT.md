...
- **AT-10의 남은 절반(installer 자체의 clean 환경 검증)을 CI에 편입하고 실제로 통과를 확인했다.** `windows-release.yml`에 Inno Setup 컴파일 → silent 설치 → 설치된 위치에서 `--self-test` → 시작 메뉴 바로가기 확인 → silent 제거까지 자동으로 수행하는 단계를 추가했다(빌드된 `setup.exe`는 `GenomeWorkbench-installer` artifact로 업로드됨). `https://github.com/dudrnjs8300/genome-workbench/actions/runs/33012146083`(windows-release #3)에서 실제로 실행해 4분 58초 만에 전체 성공, `GenomeWorkbench-installer`(42.1MB)와 `GenomeWorkbench-windows-x64`(62.5MB) 두 artifact가 모두 생성됨을 확인했다. 이 러너는 이 개발 머신과 달리 아무것도 미리 설치되어 있지 않은 진짜 clean 환경이므로, "완전히 별도의 clean Windows 환경에서의 installer 설치/실행/제거 검증"이라는 AT-10의 요구를 (영구적인 별도 사용자 계정/VM은 아니지만) 매 릴리스마다 자동으로 반복 검증하는 형태로 충족한다. 남은 것은 File association(`.gwbproj`) 자체의 GUI 수동 확인뿐이다.
