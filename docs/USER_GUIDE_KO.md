...
- 캔버스 바로 아래 **가로 스크롤바**: 확대한 상태에서 좌우로 이동할 때 쓴다(휠 대신 드래그하거나 양 끝 화살표/트랙을 클릭해도 된다). 전체 genome이 한 화면에 다 들어오면 비활성화된다.
- 그 아래 **minimap**: 클릭하거나 드래그하면 그 위치로 바로 이동한다(전체 genome 안에서 지금 보는 위치와 feature 분포를 한눈에 보여줌).
...
- **마우스 휠**로 좌우(열) 확대/축소, **Shift + 휠**로 좌우 이동. 서열이 화면보다 많으면 오른쪽 세로 스크롤바로 위아래 이동하고, 확대한 상태에서는 캔버스 아래 **가로 스크롤바**로 좌우(열) 이동한다.
