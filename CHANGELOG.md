...
- **Genome Map / Alignment View에 가로 스크롤바 추가**: 확대한 상태에서 좌우로 이동하는 방법이
  마우스 휠(Shift+휠)뿐이라 발견하기 어렵다는 피드백을 반영했다. 이제 두 화면 모두 캔버스 바로
  아래에 표준 가로 스크롤바가 있어 드래그하거나 양 끝 화살표/트랙을 클릭해서 이동할 수 있다.
  전체가 한 화면에 다 들어오면 자동으로 비활성화된다. 기존 확대/축소, minimap, Shift+휠 이동은
  그대로 동작한다.

## v0.6.1 (2026-09-22)

- **"database is locked" 오류 개선**: project 파일이 OneDrive/Dropbox 등 동기화 폴더나 네트워크
  드라이브에 있을 때 이 오류가 계속 뜬다는 사용자 제보를 조사해 두 가지를 고쳤다. SQLite 연결의
  대기 시간을 5초(기본값)에서 30초로 늘려 백신/동기화 프로그램이 파일을 잠깐 붙잡는 정도로는
  실패하지 않게 했고, 앱 전체에 전역 오류 처리기를 추가해 이런 오류가 나면(또는 다른 어떤
  예기치 못한 오류든) 조용히 사라지거나 원문 traceback이 노출되는 대신 원인과 대처법을 알려주는
  안내창이 뜨도록 했다.
